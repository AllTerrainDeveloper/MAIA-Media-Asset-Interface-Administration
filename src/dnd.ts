 ...
 * target, and other plugins can accept `allterrain-media-explorer/task` without knowing
...
export function watchShellDragVisuals( payloadType: string ): () => void {
	...
	const onStart = ( event: Event ) => sourceOf( event )?.classList.add( 'atme-is-dragging' );
	const onEnd = ( event: Event ) => sourceOf( event )?.classList.remove( 'atme-is-dragging' );
