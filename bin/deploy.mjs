 ...
 * anything written into `<checkout>/src/wp-content/plugins/allterrain-media-explorer` is
 ...
 *
 * Override the destination with ATME_DEPLOY_TARGET, or skip entirely with
 * ATME_SKIP_DEPLOY=1.
 */
...
/**
 * Marker proving a directory is an AllTerrain Media Explorer install rather than something else.
 *
 * The sync deletes files the source no longer has, so it must never be pointed at
 * a directory it does not own. Refusing unless this file is present is the guard.
 */
const OWNERSHIP_MARKER = 'allterrain-media-explorer.php';

/** Resolves the plugin directory to write to, or null when there is nothing to do. */
function resolveTarget() {
	if ( process.env.ATME_SKIP_DEPLOY === '1' ) {
		return null;
	}

	if ( process.env.ATME_DEPLOY_TARGET ) {
		return resolve( process.env.ATME_DEPLOY_TARGET );
	}
	...
	for ( const plugins of candidates ) {
		if ( existsSync( plugins ) ) {
			return join( plugins, 'allterrain-media-explorer' );
		}
	}
...
if ( ! target ) {
	console.log(
		'[allterrain-media-explorer] No local WordPress checkout found — skipping deploy. ' +
			'Set ATME_DEPLOY_TARGET to override.'
	);
	process.exit( 0 );
}
...
if ( existsSync( target ) && ! existsSync( join( target, OWNERSHIP_MARKER ) ) ) {
	console.error(
		`[allterrain-media-explorer] Refusing to sync into ${ target }: it exists but has no ${ OWNERSHIP_MARKER }.\n` +
			'That directory does not look like an AllTerrain Media Explorer install, and syncing removes files.'
	);
	process.exit( 1 );
}

for ( const required of [ 'assets/js/explorer.min.js', 'assets/js/shell.min.js' ] ) {
	if ( ! existsSync( join( root, required ) ) ) {
		console.error(
			`[allterrain-media-explorer] ${ required } is missing. Run \`npm run build\` rather than deploying alone.`
		);
		process.exit( 1 );
	}
}
...
console.log(
	`[allterrain-media-explorer] Deployed to ${ target } (${ written } file${ written === 1 ? '' : 's' } updated` +
