...
const CONTAINER = process.env.ATME_PHP_CONTAINER || 'wordpress-alcazaba-php-1';
 ...
 */
function runOnWpEnv() {
	// No Composer, by house rule — the toolchain is PHPUnit's own PHAR plus
	// the Yoast polyfills tarball, fetched into bin/ (which never ships) by
	// `bin/fetch-php-tools.mjs`. The WordPress test bootstrap finds the
	// polyfills through WP_TESTS_PHPUNIT_POLYFILLS_PATH.
	const tools = spawnSync( 'node', [ 'bin/fetch-php-tools.mjs' ], { stdio: 'inherit' } );

	if ( ( tools.status ?? 1 ) !== 0 ) {
		return tools.status ?? 1;
	}

	const inside =
		'WP_TESTS_DIR=/wordpress-phpunit ' +
		'WP_TESTS_PHPUNIT_POLYFILLS_PATH=bin/phpunit-polyfills/phpunitpolyfills-autoload.php ' +
		'php bin/phpunit.phar --configuration tests/phpunit/phpunit.xml.dist ' +
		args.join( ' ' );
...
const forced = process.env.ATME_PHP_BACKEND;

if ( forced && ! [ 'alcazaba', 'wp-env' ].includes( forced ) ) {
	process.stderr.write(
		`[${ SLUG }] Unknown ATME_PHP_BACKEND "${ forced }". Use "alcazaba" or "wp-env".\n`
	);
	process.exit( 1 );
}
