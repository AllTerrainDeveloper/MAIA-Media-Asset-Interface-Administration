...
if ! [[ "$new" =~ ^[0-9]+\.[0-9]+\.[0-9]+(-[A-Za-z0-9.]+)?$ ]]; then
	echo "error: expected X.Y.Z or X.Y.Z-prerelease (without v)" >&2
	exit 1
fi
...
#
# Anything else dirty still aborts to keep unrelated work out of the release.
# That deliberately includes assets/ — the built bundles
...
dirty=$(git status --porcelain --untracked-files=no | grep -vE '^.{3}(readme\.txt$|package\.json$|package-lock\.json$|allterrain-media-explorer\.php$)' || true)
if [[ -n "$dirty" ]]; then
...
# normally.
if git diff HEAD --quiet -- package.json package-lock.json allterrain-media-explorer.php; then
	bump_committed=1
else
	bump_committed=0
fi

if [[ "$pkg" == "$new" && "$header" == "$new" && "$constant" == "$new" && "$stable" == "$new" && "$lock" == "$new" && "$lock_root" == "$new" && "$bump_committed" == "1" ]]; then
	...
	if ! git diff HEAD --quiet -- readme.txt; then
	...
	if git diff HEAD --quiet; then
		...
		git add -- package.json package-lock.json allterrain-media-explorer.php readme.txt
