...
function payloadFor( source: HTMLElement, type = 'allterrain-media-explorer/task' ): DragPayload {
...
describe( 'fallback drag manager', () => {
	...
	it( 'does not lift until the pointer passes the threshold', () => {
		...
		expect( source.classList.contains( 'atme-is-dragging' ) ).toBe( false );

		document.dispatchEvent( pointer( 'pointermove', 40, 10 ) );

		expect( source.classList.contains( 'atme-is-dragging' ) ).toBe( true );
	...
	it( 'cancels on Escape and leaves the source unmarked', () => {
		...
		expect( source.classList.contains( 'atme-is-dragging' ) ).toBe( false );
		expect( document.querySelector( '.atme-drag-ghost' ) ).toBeNull();
